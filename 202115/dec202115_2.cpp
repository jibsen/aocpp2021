//
// Advent of Code 2021, day 15, part two
//
...
void expand_map(Map &map)
{
	for (auto &row : map) {
		...
		for (auto x = size; x < row.size(); ++x) {
			row[x] = (row[x - size] % 9) + 1;
		}
	}
	...
	for (auto y = size; y < map.size(); ++y) {
		map[y].reserve(map[y - size].size());

		std::ranges::transform(map[y - size], std::back_inserter(map[y]),
			[](auto risk) {
				return (risk % 9) + 1;
			}
		);
