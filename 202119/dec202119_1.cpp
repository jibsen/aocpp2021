//
// Advent of Code 2021, day 19, part one
//
...
int main()
{
	...
	std::unordered_set<Vec3, ArrayHash> final_beacons;

	for (int i = 0; i < scanners.size(); ++i) {
		auto trans_to_zero = translations[i];
		auto perm_to_zero = perms[i];

		for (const auto &beacon : scanners[i]) {
			final_beacons.insert(transform_point(beacon, perm_to_zero, trans_to_zero));
		}
	}

	std::cout << final_beacons.size() << '\n';
}
